...
# args <- list("scenario" = "ssp126")

# For navigation
dir1 <- "data/ISIMIP3a/adapted/distributions"
dir2 <- paste0("data/ISIMIP3a/adapted/relative/", args$scenario)
...
H <- bSpline(TEMP_eval, knots = basis_par$Knots[2:(length(basis_par$Knots) - 1)], 
             degree = basis_par$Order, intercept = FALSE)
...
if (args$scenario == "historical") {
  pdates <- paste0("data/ISIMIP3a/adapted/distributions/", 
                   "base_season_parameters.csv") %>% 
    read_csv(col_types = cols()) %>% 
    mutate(PDAY = paste(YEAR_P, MONTH_P, DAY_P, sep = "-") %>% ymd() %>% yday()) %>% 
    rename(Year = YEAR_P) %>% 
    select(County, Year, PDAY)
  
  pdates <- expand_grid(Model = models, 
                        data = list(pdates)) %>% 
    unnest(data)
} else {
  pdates <- map_df(models, function(m) {
        paste0(dir1, "/", m, "/", args$scenario, "/adapted_seasons.csv") %>% 
          read_csv(col_types = cols()) %>% 
          mutate(PDAY = yday(DATE_P), 
                 Model = m) %>% 
          dplyr::select(Model, County, Year, PDAY)
      }) %>% 
    filter(Year != 2100)
}
...
pdate_coef <- read_rds("data/historical/CUB_results/CUB_pday.rds") %>% 
  pull(Value)
prcp_coef <- paste0("data/historical/CUB_feffects_raw.csv") %>% 
...
pdates2 <- pdates %>% 
  split(~ Model) %>% 
  map(function(df) {
      df <- pivot_wider(df, names_from = "Year", values_from = "PDAY")
...
cc_hyb <- read_csv("data/historical/CUB_reffects_raw.csv", 
...
cc_fix <- paste0("data/historical/CUB_feffects_raw.csv") %>% 
  read_csv() %>% 
...
cl <- makeCluster(20L, type = "FORK", 
                  outfile = paste0("logs/adapted_", args$scenario, 
                                   "_", args$basis, ".log"))
...
ref <- parLapplyLB(cl, 1:(dim(cc_hyb)[3]), function(i) {
  ...
  write_rds(global, paste0(dir2, "/global_bootstrap", i_tag, ".rds"))
  rm(global); gc()
  ...
  write_rds(group, paste0(dir2, "/group_bootstrap", i_tag, ".rds"))
  rm(group); gc()
