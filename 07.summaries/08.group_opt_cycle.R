...
hist <- list.files("data/ISIMIP3a/cycle_adapted/relative/historical", 
...
ssp <- paste0("ssp", c(126, 370, 585)) %>% 
  map(function(s) {
    cat(s, "\n")
    list.files(paste0("data/ISIMIP3a/cycle_adapted/relative/", s), 
               "group_bootstrap[0-9]{4}", full.names = TRUE) %>% 
      map(read_rds) %>% 
      abind(along = 5)
  })
...
write_rds(list(periods, periods_post, ensemble, ensemble_post),
          "data/climate_summaries/group_opt_cycle.rds")
