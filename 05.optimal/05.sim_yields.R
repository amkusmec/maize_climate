...
# For navigation
dir1 <- "data/ISIMIP3a/cycle_adapted/distributions"
dir2 <- paste0("data/ISIMIP3a/cycle_adapted/relative/", args$scenario)
...
cal <- list.files("data/ISIMIP3a/cycle_adapted/distributions", 
                  "season_calendar\\.csv", full.names = TRUE, recursive = TRUE)
pdates <- cal[str_detect(cal, args$scenario)] %>% 
  map_df(function(f) {
      read_csv(f, col_types = cols()) %>% 
        mutate(Model = str_split(f, "/") %>% unlist() %>% `[`(8))
    }) %>% 
  select(Model, County, Year, PDAY)
...
cl <- makeCluster(20L, type = "FORK", 
                  outfile = paste0("logs/adapted_", args$scenario, ".log"))
