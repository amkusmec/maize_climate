...
gdd <- lapply(1:nrow(seasons), function(i) {
  ...
  file_list <- paste0("data/historical/prism_dailys/PRISM_", seasons$Cells[[i]], "_daily.csv")
...
write_csv(gdd, "data/ISIMIP3a/reference_cal/estimated_season_lengths.csv")


s2 <- seasons %>% 
  unnest(data) %>% 
  select(County:Harvested) %>% 
  inner_join(gdd, by = c("State", "County", "Year" = "YEAR")) %>% 
  mutate(PDAY = yday(Planted)) %>% 
  group_by(State, County) %>% 
  summarise(across(GDD:PDAY, ~ mean(.x)), 
            .groups = "drop") %>% 
  mutate(PDAY = floor(PDAY))

write_csv(s2, "data/ISIMIP3a/reference_cal/season_parameters.csv")
