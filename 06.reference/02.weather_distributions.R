...
dir1 <- paste0("data/ISIMIP3a/reference_cal/distributions/", args$model)
if (!dir.exists(dir1)) dir.create(dir1)
dir2 <- paste0(dir1, "/", args$scenario)
if (!dir.exists(dir2)) dir.create(dir2)

hist_par <- read_csv("data/ISIMIP3a/reference_cal/season_parameters.csv", 
                     col_types = cols())
  ...
  dists <- mclapply(file_list, function(f) {
    dailys <- read_csv(f, col_types = cols()) %>% 
      mutate(DATE = ymd(paste(YEAR, MONTH, DAY, sep = "-")), 
             YDAY = yday(DATE)) %>% 
      filter(YDAY >= hist_par$PDAY[i]) %>% 
      mutate(GDD = 0.5*(if_else(TMIN < 10, 10, TMIN) + 
                          if_else(TMAX < 10, 10, if_else(TMAX > 30, 30, TMAX))) - 10) %>% 
      group_by(YEAR) %>% 
      mutate(GDDc = cumsum(GDD)) %>% 
      filter(GDDc < hist_par$GDD[i]) %>% 
      ungroup()
